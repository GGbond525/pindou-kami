...
def get_db():
    if DB_URL:
        try:
            ...
            c.q = q; c.fo = lambda s,p=None: q(s,p).fetchone(); c.fa = lambda s,p=None: q(s,p).fetchall()
            return c
        except: pass
    import sqlite3
...
def reg():
    ...
    db=get_db()
    if db.fo(f"SELECT id FROM users WHERE username={PH}",(u,)): db.close(); return jsonify({"s":False,"m":"用户名已存在"})
    h=hashlib.sha256(p.encode()).hexdigest(); n=datetime.datetime.now().strftime('%Y-%m-%d %H:%M')
    db.q(f"INSERT INTO users (username,password,created_at) VALUES ({PH},{PH},{PH})",(u,h,n))
...
def lgn():
    ...
    db=get_db(); us=rd(db.fo(f"SELECT * FROM users WHERE username={PH} AND password={PH}",(u,h))); db.close()
...
def act():
    ...
    db=get_db(); us=rd(db.fo(f"SELECT kami_code,member_expire_at FROM users WHERE username={PH}",(u,)))
    ...
    r=rd(db.fo(f"SELECT * FROM kami_codes WHERE code={PH} AND status='unused' AND card_type LIKE 'm%'",(c,)))
    if not r:
        used=db.fo(f"SELECT * FROM kami_codes WHERE code={PH} AND status='used'",(c,))
    ...
    db.q(f"UPDATE kami_codes SET status='used', used_by={PH}, used_at={PH} WHERE id={PH}",(u,ns,r["id"]))
    db.q(f"UPDATE users SET kami_code={PH}, member_type={PH}, activated_at={PH}, member_expire_at={PH} WHERE username={PH}",(c,r["duration"],ns,exp,u))
    db.commit(); db.close()
...
def act_ag():
    ...
    db=get_db(); us=rd(db.fo(f"SELECT is_agent,agent_kami_code FROM users WHERE username={PH}",(u,)))
    if us and us.get("is_agent") and us.get("agent_kami_code"): db.close(); return jsonify({"s":False,"m":"已绑定代理卡密"})
    r=rd(db.fo(f"SELECT * FROM kami_codes WHERE code={PH} AND status='unused' AND card_type LIKE 'a%'",(c,)))
    if not r:
        used=db.fo(f"SELECT * FROM kami_codes WHERE code={PH} AND status='used'",(c,))
    ...
    db.q(f"UPDATE kami_codes SET status='used', used_by={PH}, used_at={PH} WHERE id={PH}",(u,ns,r["id"]))
    db.q(f"UPDATE users SET is_agent=1, agent_kami_code={PH}, wechat_id={PH}, agent_quota={PH} WHERE username={PH}",(c,w,qa,u))
    db.commit(); db.close(); return jsonify({"s":True,"m":"升级代理成功","aq":qa})

@app.route('/api/check', methods=['POST'])
def chk():
    u=request.get_json().get('username','')
    db=get_db(); us=rd(db.fo(f"SELECT * FROM users WHERE username={PH}",(u,))); db.close()
...
def ag_gen():
    ...
    db=get_db(); us=rd(db.fo(f"SELECT * FROM users WHERE username={PH} AND is_agent=1",(u,)))
    if not us: db.close(); return jsonify({"s":False,"m":"无权限"}),403
    ud=rd(db.fo(f"SELECT COUNT(*) as c FROM kami_codes WHERE created_by={PH}",(u,)))
    ...
    for _ in range(n):
        c=gc(16)
        try: db.q(f"INSERT INTO kami_codes (code,card_type,duration,duration_days,created_at,created_by,status) VALUES ({PH},'member_perm','perm',99999,{PH},{PH},'unused')",(c,ns,u)); cs.append(c)
        except: pass
...
def ag_info():
    ...
    ud=rd(db.fo(f"SELECT COUNT(*) as c FROM kami_codes WHERE created_by={PH}",(u,))); uc=ud["c"] if ud else 0
...
def ag_list():
    ...
    db=get_db(); rs=db.fa(f"SELECT code,created_at,status,used_at FROM kami_codes WHERE card_type LIKE 'm%' AND created_by={PH} {wh} ORDER BY id DESC LIMIT 200",(u,))
...
def ag_wx():
    ...
    db.q(f"UPDATE users SET wechat_id={PH} WHERE username={PH}",(d.get('wechat_id',''),d.get('username','')))
...
def ad_gen():
    ...
    for _ in range(n):
        c=gc(cl)
        try: db.q(f"INSERT INTO kami_codes (code,card_type,duration,duration_days,created_at,status) VALUES ({PH},{PH},{PH},{PH},{PH},'unused')",(c,ct,dr,du["d"],ns)); cs.append(c)
        except: pass
...
def ad_ls():
    ...
    rs=db.fa(f"SELECT * FROM kami_codes WHERE status='{tb}' {wt} ORDER BY id DESC LIMIT 500")
    tl=rd(db.fo("SELECT COUNT(*) as c FROM kami_codes")); ul=rd(db.fo("SELECT COUNT(*) as c FROM kami_codes WHERE status='unused'"))
    sl=rd(db.fo("SELECT COUNT(*) as c FROM kami_codes WHERE status='used'")); mc=rd(db.fo("SELECT COUNT(*) as c FROM kami_codes WHERE card_type LIKE 'm%' AND status='unused'"))
    ac=rd(db.fo("SELECT COUNT(*) as c FROM kami_codes WHERE card_type LIKE 'a%' AND status='unused'"))
    db.close()
...
def ad_dl():
    d=request.get_json()
    if d.get('pw')!=ADMIN_PASSWORD: return jsonify({"s":False}),403
    db=get_db(); db.q(f"DELETE FROM kami_codes WHERE id={PH}",(d['id'],)); db.commit(); db.close()
...
def ad_us():
    if request.args.get('pw','')!=ADMIN_PASSWORD: return jsonify({"s":False}),403
    db=get_db()
    rs=db.fa("SELECT username,created_at,kami_code,member_type,member_expire_at,activated_at,is_agent,agent_kami_code,wechat_id,agent_quota FROM users ORDER BY id DESC LIMIT 200")
...
def gwx():
    ...
    db=get_db(); r=rd(db.fo(f"SELECT created_by FROM kami_codes WHERE code={PH} AND created_by IS NOT NULL",(cd,)))
    if r:
        a=rd(db.fo(f"SELECT wechat_id FROM users WHERE username={PH}",(r["created_by"],)))
