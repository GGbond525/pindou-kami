...
if DB_URL:
    ...
    def gd():
        ...
        cr.execute("CREATE TABLE IF NOT EXISTS kami_codes (id SERIAL PRIMARY KEY, co TEXT UNIQUE NOT NULL, cd TEXT NOT NULL DEFAULT 'm_perm', dr TEXT DEFAULT 'perm', dd INTEGER DEFAULT 99999, ca TEXT, cb TEXT, ub TEXT, ua TEXT, st TEXT DEFAULT 'unused')")
        cr.execute("CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, un TEXT UNIQUE NOT NULL, pw TEXT NOT NULL, ca TEXT, kc TEXT, mt TEXT, aa TEXT, me TEXT, ia INTEGER DEFAULT 0, ak TEXT, wi TEXT, aq INTEGER DEFAULT 50)")
        c.commit(); return c
    def q(c,s,p=None):
    ...
    def gd():
        ...
        c.execute("CREATE TABLE IF NOT EXISTS kami_codes (id INTEGER PRIMARY KEY AUTOINCREMENT, co TEXT UNIQUE NOT NULL, cd TEXT NOT NULL DEFAULT 'm_perm', dr TEXT DEFAULT 'perm', dd INTEGER DEFAULT 99999, ca TEXT, cb TEXT, ub TEXT, ua TEXT, st TEXT DEFAULT 'unused')")
        c.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, un TEXT UNIQUE NOT NULL, pw TEXT NOT NULL, ca TEXT, kc TEXT, mt TEXT, aa TEXT, me TEXT, ia INTEGER DEFAULT 0, ak TEXT, wi TEXT, aq INTEGER DEFAULT 50)")
        for x in ["kc","mt","me","dd"]:
            try: c.execute(f"ALTER TABLE users ADD COLUMN {x} TEXT")
            except: pass
            try: c.execute(f"ALTER TABLE kami_codes ADD COLUMN {x} TEXT")
            except: pass
            try: c.execute(f"ALTER TABLE kami_codes ADD COLUMN dd INTEGER")
            except: pass
        c.commit(); return c
...
def reg():
    ...
    db=gd()
    if fo(db,f"SELECT id FROM users WHERE un={PH}",(u,)): db.close(); return jsonify({"s":False,"m":"用户名已存在"})
    h=hashlib.sha256(p.encode()).hexdigest(); n=datetime.datetime.now().strftime('%Y-%m-%d %H:%M')
    db.execute(f"INSERT INTO users (un,pw,ca) VALUES ({PH},{PH},{PH})",(u,h,n))
...
def lgn():
    ...
    db=gd(); us=rd(fo(db,f"SELECT * FROM users WHERE un={PH} AND pw={PH}",(u,h))); db.close()
...
def act():
    ...
    db=gd(); us=rd(fo(db,f"SELECT kc,me FROM users WHERE un={PH}",(u,)))
    if us and us.get("kc"):
    ...
    r=rd(fo(db,f"SELECT * FROM kami_codes WHERE co={PH} AND st='unused' AND cd LIKE 'm_%'",(c,)))
    if not r:
        used=fo(db,f"SELECT * FROM kami_codes WHERE co={PH} AND st='used'",(c,))
    ...
    db.execute(f"UPDATE kami_codes SET st='used', ub={PH}, ua={PH} WHERE id={PH}",(u,ns,r["id"]))
    db.execute(f"UPDATE users SET kc={PH}, mt={PH}, aa={PH}, me={PH} WHERE un={PH}",(c,r["dr"],ns,exp,u))
    db.commit(); db.close()
...
def act_ag():
    ...
    db=gd(); us=rd(fo(db,f"SELECT ia,ak FROM users WHERE un={PH}",(u,)))
    if us and us.get("ia") and us.get("ak"): db.close(); return jsonify({"s":False,"m":"已绑定代理卡密"})
    r=rd(fo(db,f"SELECT * FROM kami_codes WHERE co={PH} AND st='unused' AND cd LIKE 'a_%'",(c,)))
    if not r:
        used=fo(db,f"SELECT * FROM kami_codes WHERE co={PH} AND st='used'",(c,))
    ...
    db.execute(f"UPDATE kami_codes SET st='used', ub={PH}, ua={PH} WHERE id={PH}",(u,ns,r["id"]))
    db.execute(f"UPDATE users SET ia=1, ak={PH}, wi={PH}, aq={PH} WHERE un={PH}",(c,w,qa,u))
    db.commit(); db.close(); return jsonify({"s":True,"m":"升级代理成功","aq":qa})

@app.route('/api/check', methods=['POST'])
def chk():
    u=request.get_json().get('username','')
    db=gd(); us=rd(fo(db,f"SELECT * FROM users WHERE un={PH}",(u,))); db.close()
...
def ag_gen():
    ...
    db=gd(); us=rd(fo(db,f"SELECT * FROM users WHERE un={PH} AND ia=1",(u,)))
    if not us: db.close(); return jsonify({"s":False,"m":"无权限"}),403
    ud=rd(fo(db,f"SELECT COUNT(*) as c FROM kami_codes WHERE cb={PH}",(u,))); uc=ud["c"] if ud else 0
    ...
    for _ in range(n):
        c=gc(16)
        try: db.execute(f"INSERT INTO kami_codes (co,cd,dr,dd,ca,cb,st) VALUES ({PH},'m_perm','perm',99999,{PH},{PH},'unused')",(c,ns,u)); cs.append(c)
        except: pass
...
def ag_list():
    ...
    db=gd(); rs=fa(db,f"SELECT cd,ca,st,ua FROM kami_codes WHERE cd LIKE 'm_%' AND cb={PH} {wh} ORDER BY id DESC LIMIT 200",(u,))
    db.close(); return jsonify({"s":True,"cs":[dict(r) for r in rs]})

@app.route('/api/agent/set_wx', methods=['POST'])
def ag_wx():
    d=request.get_json(); db=gd()
    db.execute(f"UPDATE users SET wi={PH} WHERE un={PH}",(d.get('wi',''),d.get('username','')))
...
def ad_gen():
    ...
    for _ in range(n):
        c=gc(cl)
        try: db.execute(f"INSERT INTO kami_codes (co,cd,dr,dd,ca,st) VALUES ({PH},{PH},{PH},{PH},{PH},'unused')",(c,ct,dr,du["d"],ns)); cs.append(c)
        except: pass
...
def ad_ls():
    ...
    rs=fa(db,f"SELECT * FROM kami_codes WHERE st='{tb}' {wt} ORDER BY id DESC LIMIT 500")
    def c(sql): r=fo(db,sql); return r["c"] if r else 0
    tl=c("SELECT COUNT(*) as c FROM kami_codes"); ul=c("SELECT COUNT(*) as c FROM kami_codes WHERE st='unused'")
    sl=c("SELECT COUNT(*) as c FROM kami_codes WHERE st='used'")
    mc=c("SELECT COUNT(*) as c FROM kami_codes WHERE cd LIKE 'm_%' AND st='unused'")
    ac=c("SELECT COUNT(*) as c FROM kami_codes WHERE cd LIKE 'a_%' AND st='unused'")
    db.close()
...
def ad_dl():
    d=request.get_json()
    if d.get('pw')!=ADMIN_PASSWORD: return jsonify({"s":False}),403
    db=gd(); db.execute(f"DELETE FROM kami_codes WHERE id={PH}",(d['id'],)); db.commit(); db.close()
...
def ad_us():
    if request.args.get('pw','')!=ADMIN_PASSWORD: return jsonify({"s":False}),403
    db=gd()
    rs=fa(db,"SELECT un,ca,kc,mt,me,aa,ia,ak,wi,aq FROM users ORDER BY id DESC LIMIT 200")
...
def gwx():
    ...
    db=gd(); r=rd(fo(db,f"SELECT cb FROM kami_codes WHERE co={PH} AND cb IS NOT NULL",(cd,)))
    if r:
        a=rd(fo(db,f"SELECT wi FROM users WHERE un={PH}",(r["cb"],)))
